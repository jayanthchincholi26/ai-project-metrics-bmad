...
# Story 2.8: git already ignores this hook's exit code (post-checkout is
...
if command -v uv >/dev/null 2>&1; then
    uv run tools/hooks/git/post-checkout.py "$@"
else
    echo "warning: uv not found on PATH - metrics capture skipped for this post-checkout hook" >&2
fi
