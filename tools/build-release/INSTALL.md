...
| --- | --- | --- |
| Git | Capture hooks are git hooks; branch-per-story is assumed for time attribution | `git --version` |
| Python 3.8+ | All tooling is single-file Python scripts | `python --version` |
| uv | Every script runs via `uv run` (no venv management needed) | `uv --version` |
| Claude Code | AI-session capture (the default `ai_tool: claude-code` adapter) | the VS Code extension or CLI |
| JIRA via MCP *(only if your project uses JIRA)* | Kickoff auto-fills points/goal/sprint through a JIRA MCP server — OAuth, **no personal API token** | see "JIRA setup" below |
