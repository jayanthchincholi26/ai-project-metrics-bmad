...
3. Normalize the response (`issues.nodes[0].fields`) to the AD-4 shape — same rules Story 1.3's adapter implements:
   ...
   - **sprint_start_date** / **sprint_end_date** (Story 6.5) ← the **same chosen sprint object** above (never re-select independently — an issue can carry more than one sprint entry, its full history, and the two must agree on which one) — take its `startDate`/`endDate` if present. A sprint that hasn't started yet (`state: "future"`) carries no date keys at all; both are `null` in that case, never invented.
...
5. **Degradation chain — kickoff is never blocked** (FR5):
   ...
   - **No Confluence MCP tools available in this session** → say so plainly. Then, only if `CONFLUENCE_BASE_URL` (including `/wiki` for Cloud sites), `CONFLUENCE_EMAIL`, and `CONFLUENCE_API_TOKEN` are all set in the environment, fall back to the Story 1.4 script:
     ...
     ```
     same normalized ack, surface stderr verbatim on failure. **This script path is also the only way to get real points/sprint auto-fill from labels** — it reads them directly via the Confluence REST API, unaffected by the MCP gap above. Otherwise fall back to the plain step-4 ask for points/goal/description, unassisted — **except sprint**, same rule as immediately above: never offer the "None" option here, ask for a confirmed non-empty value as plain free text (found live during this exact fallback path, Story 1.8).
...
```
uv run tools/adapters/docs-only/main.py --repo-root <repo-root> [--name "<name>"] --points <N> --goal "<goal>" [--sprint "<sprint>"] [--sprint-start-date "<date>"] [--sprint-end-date "<date>"] [--description "<text>"] [--source-of-truth jira|confluence|docs-only] --ai-tool <resolved ai_tool> [--points-estimated <raw Phase-1 estimate>] [--jira-issue-key <KEY>]
```

(`--source-of-truth` defaults to `docs-only`; the JIRA/Confluence flows pass their backend so the manifest records which one supplied the values. `--ai-tool` carries the step-1 resolved value — the manifest field AI-session capture producers read to pick their `ai.<tool>.*` event namespace. `--points-estimated` carries step 3's raw `phase1_points`, if any was produced — omit it entirely when step 3 returned null; never pass the developer's confirmed `--points` value here, the two must stay distinct per AD-6a. `--name` carries step 4.0's answer — **docs-only only** (Story 1.7); the JIRA/Confluence variants never pass it, and the writer defaults it to `null`. `--sprint` is omitted entirely when step 4.4 resolved to "none" — **docs-only only**; the JIRA/Confluence variants always pass a confirmed non-empty value, exactly as before Story 1.7. `--sprint-start-date`/`--sprint-end-date` (Story 6.5) carry step 4a.3's extracted dates — **JIRA only**, omitted whenever the chosen sprint object had no dates (docs-only/Confluence never have any to pass, and the writer defaults both to `null`). `--jira-issue-key` carries step 4a's issue key — **JIRA only** (Story 5.4); the Confluence/docs-only variants never pass it, and the writer defaults it to `null` since there is no Jira parent to attach a later defect subtask to.)
