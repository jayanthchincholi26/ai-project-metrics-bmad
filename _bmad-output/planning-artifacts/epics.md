...
> 🔨 **Built and live-verified 2026-07-21 on `epic-6-jira-lifecycle-sync`, not yet merged to `main`** — real gap closed first: `tools/snapshot-assembler/main.py`'s `pm_metrics` now carries Story 6.5's `sprint_start_date`/`sprint_end_date` through (they were being silently dropped at every close before this). New `tools/dashboard/main.py` "Sprint Rollups" table groups stories by `pm_metrics.sprint`: Sprint Name, Start Date, End Date, Story Count, and an Overall Status of `Ended` / `Active or upcoming` / `Unknown` computed from the sprint's own end date vs. today — not a story-done/open count, which is impossible to compute honestly from this pipeline's closed-only snapshot data (see the authoring corrections below). A "No Sprint" bucket row covers stories with no sprint value; the whole section is omitted when no story has a sprint at all. Live-verified in a real scratch repo running the actual `snapshot-assembler`/`dashboard` tools: two stories sharing one sprint (real past dates → `Ended`), a single-story sprint with a future end date (→ `Active or upcoming`), a sprint with no captured dates (→ `unknown`/`Unknown`), a no-sprint story, and a separate all-docs-only repo confirming the section is fully omitted. 392 tests passing (up from 375, +17). This was the last of the three stories (6.4, 6.5, 6.6) queued before cutting the release for the user's own end-to-end testing. Awaiting the user's own end-to-end testing before this branch merges to `main`.
...
> 🔨 **Built and live-verified 2026-07-24 on `epic-6-jira-lifecycle-sync`, not yet merged to `main`** — a `PreToolUse` hook now denies both close commands outright for a JIRA-backed story until `story-close`'s own step 6 has created a single-use `.story-close-ack` marker immediately before running the real close command; a raw pasted command gets redirected via `additionalContext` (confirmed, via real research into Claude Code's current hook docs, that this is the field the assistant model actually sees — `permissionDecisionReason` is UI-only, and getting these two backwards would have silently reproduced the exact bug this story fixes). Real-subprocess verification (piping real stdin JSON to the actual script in a scratch repo, not just pytest's mocked fixtures) confirmed every path: deny with no marker, allow-and-consume with one present, denied again immediately after (single-use), never gated for docs-only or `--dry-run`. 403 tests passing (+11). **Honest gap, not glossed over:** this repo has no hooks wired on itself, so the real Claude Code harness's own enforcement (actual tool-call denial, actual `additionalContext` visibility to the model) could not be verified from this session — that final check is the user's own live retest of the exact scenario that surfaced GitHub #52. Awaiting that retest, the rest of Epic 6's own testing, and the user's own end-to-end testing before this branch merges to `main`.

**Status:** built 2026-07-24, in review — deferred fix requested 2026-07-23 ("workaround for now, fix later"), built the same week once the user asked to work on it. Not yet confirmed at the real-Claude-Code-harness level (see status blockquote above) — the user's own retest of GitHub #52's exact scenario is the remaining verification step.
