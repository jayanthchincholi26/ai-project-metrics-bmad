...
- **Logging a real, verified-real review finding as a defect:** once a finding is confirmed real (against the diff) and fixed, follow `.claude/skills/log-review-defect/SKILL.md` — this is a side-effect of the fix, not a separate task for the developer to remember. (Originally an inline instruction here, Story 5.4; generalized into a shipped skill and de-duplicated by Story 6.3, since the inline copy was never actually reaching real end users of the packaged tool — only this repo's own dogfooding.)
